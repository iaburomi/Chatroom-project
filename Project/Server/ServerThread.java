...
import Project.Commons.RoomResultPayload;

/**
 * A server-side representation of a single client
 */
public class ServerThread extends Thread {
    private static final String sender = null;
    ...
    void processPayload(Payload p) {
        switch (p.getPayloadType()) {
            case SWITCH_ROOM:
                ...
                String targetRoom = p.getTargetRoom();
                if (targetRoom != null && !targetRoom.isEmpty()) {
                    Room.joinRoom(targetRoom, this);
                }
                break;
            case CONNECT:
            ...
            //iaa47
            //11/13/23
                String category = "Random Category"; 
                String question = "Random Question";
                List<String> answerOptions = List.of("Option 1", "Option 2", "Option 3", "Option 4"); 
                int roundDuration = 60;
                Payload triviaPayload = new Payload();
                triviaPayload.setPayloadType(PayloadType.START_ROUND);
                triviaPayload.setCategory(category);
                triviaPayload.setQuestion(question);
                triviaPayload.setAnswerOptions(answerOptions);
                triviaPayload.setRoundDuration(roundDuration);
                currentRoom.sendMessage(this, triviaPayload);
                sendTriviaInfo(sender, triviaPayload);
                break;
            case ANSWER:
            handlePlayerAnswer(sender, p);
    ...
    private boolean checkAllPlayersAnswered() {
        return false;
    }
    ...
    private void handlePlayerAnswer(String sender2, Payload p) {
    }
    ...
    public void sendMessage(String string, String format) {
    }
    ...
    public boolean sendConnectionStatus(String clientName2, boolean isConnected) {
        return false;
    }

    public boolean sendTriviaInfo(String clientName2, Payload payload) {
        return false;
    }

    public boolean send(String from, String triviaInfo) {
        return false;
    }
}